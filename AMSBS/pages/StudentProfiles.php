...
$query = "SELECT * FROM student_profiles WHERE college_id = $collegeid ORDER BY lname ASC";
$students = $db->query($query);
$StudentProfiles = $students->fetch_assoc();
...
<html lang="en">
  <head>
    ...
    <title>AMSBS Student Profiles</title>
    ...
    <div style="width:50px;"></div>
      <div class="row w-100">
        <div class="col-12 p-2">
            <div class="d-flex justify-content-start align-items-center BoxContainers p-2">
                <div>YEAR & SECTION:</div>
<div>
    <form method="POST" action="">
        <!-- Changed container classes here -->
        <div class="sections-container ms-2 d-flex flex-wrap align-items-center">
            <?php
            ...
            <div class="form-check me-3">
                <input type="checkbox" 
                       id="section_<?php echo $section; ?>" 
                       class="form-check-input section-checkbox" 
                       name="section[]" 
                       value="<?php echo $section; ?>">
                <label class="form-check-label" for="section_<?php echo $section; ?>">
                    <?php echo $section; ?>
                </label>
            </div>
            <?php endwhile; ?>
            ...
            <div class="w-100">
                <button type="submit" class="btn btn-dark btn-sm mt-2" style="border-radius: 0;">SELECT</button>
            </div>
        </div>
    </form>
</div>
            ...
            <div class="d-flex justify-content-between my-2">
                <div class="w-100 d-flex">
                    ...
                    <div class="input-group">
                        <input style="border-radius: 0;" type="text" class="form-control" placeholder="Name / ID No." aria-describedby="button-addon2">
                        <button style="border-radius: 0;" class="btn btn-outline-secondary" type="button" id="button-addon2">Search</button>
                      </div>
                    </div>
                    <div class="w-100">

                    </div>
                    <div class="w-100 text-end"><button class="btn btn-warning" style="border-radius: 0;">
            ...
            <div>
                <table class="mt-2 mb-2 table table-bordered align-middle table-striped">
                    <thead class="table-dark">
                      <tr>
                        <th class="text-center p-1 fw-normal">
                            <form>
                                <input type="checkbox">
                            </form>
                        </th>
                        <th class="p-1 fw-normal text-center">#</th>
                        <th class="p-1 fw-normal">NAME</th>
                        <th class="p-1 fw-normal">SEX</th>
                        <th class="p-1 fw-normal">ID NO.</th>
                        <th class="p-1 fw-normal">YEAR & SECTION</th>
                        <th class="p-1 fw-normal">ACTIONS</th>
                      </tr>
                      <tbody>
...
while ($student = $students->fetch_assoc()) {
    $fullName = strtoupper($student['lname'] . ', ' . $student['fname'] . ' ' . ($student['mname'] ? strtoupper(substr($student['mname'], 0, 1)) . '.' : ''));

    echo "<tr>";
    echo "<td class='text-center p-1'><input type='checkbox'></td>";
    echo "<td class='p-1 text-center'>" . $index++ . "</td>";
    echo "<td class='p-1'>" . htmlspecialchars($fullName) . "</td>";
    echo "<td class='p-1'>" . htmlspecialchars(strtoupper(substr($student['sex'], 0, 1))) . "</td>";
    echo "<td class='p-1'>" . htmlspecialchars($student['stud_id']) . "</td>";
    echo "<td class='p-1'>" . htmlspecialchars($student['section']) . "</td>";
echo "<td class='p-1'>
  <div>
    <a href='StudentRecords.php?id=" . urlencode($student['stud_id']) . "' class='btn btn-success btn-sm' style='border-radius: 0;'>View Records</a>
    <button class='btn btn-warning btn-sm download-barcode' data-id='{$student['stud_id']}' style='border-radius: 0;'>Download Barcode</button>
  </div>
  <svg id='barcode_{$student['stud_id']}' class='barcode-svg d-none'></svg>
</td>";
...
document.querySelectorAll(".barcode-svg").forEach(svg => {
  const id = svg.id.split("_")[1]; // extract student ID
  JsBarcode(svg, id, {
    format: "CODE128",
    displayValue: true,
    width: 2,
...
// Convert SVG to JPEG and trigger download
...
// Add download handler to buttons
document.querySelectorAll(".download-barcode").forEach(button => {
  button.addEventListener("click", () => {
    const studentId = button.getAttribute("data-id");
    const svg = document.getElementById("barcode_" + studentId);
    downloadSVGAsJPG(svg, "barcode_" + studentId);
  });
});
</script>
